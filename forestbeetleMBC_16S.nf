...
Channel
	.fromFilePairs( "fq_16S/*_L001_R{1,2}_001.fastq", flat:true )
	.ifEmpty { error "Cannot find any matching: ${params.reads}" }
	.set { read_pairs }

// Step 1.0 - Merge forward and reverse reads with USEARCH
process merge {
	publishDir './output_16S_1.0_merged', mode: 'copy', overwrite: true
...
process fastqc_merged {
	publishDir './output_16S_1.1_fastqc_merged', mode: 'copy', overwrite: true
...
process multiqc_merged {
	publishDir './output_16S_1.2_multiqc_merged', mode: 'copy', overwrite: true
...
process cutadapters {
	publishDir './output_16S_2.0_adapters_trimmed', mode: 'copy', overwrite: true
	...
	"""
	cutadapt -a file:$PWD/adapters_16S/${sample_id}_adapters.fasta -o ${sample_id}.adapters_trimmed.fastq $merged_reads_2
	"""
}

// Step 2.1 - report quality control with fastqc
process fastqc_cutadapters {
        publishDir './output_16S_2.1_fastqc_cutadapters', mode: 'copy', overwrite: true
...
process multiqc_cutadapters {
        publishDir './output_16S_2.2_multiqc_cutadapters', mode: 'copy', overwrite: true
...
process cutprimers {
	publishDir './output_16S_3.0_primers_trimmed', mode: 'copy', overwrite: true
	...
	"""
	cutadapt -M 165 -m 124 --no-indels -a "AGACGAGAAGACCCTATAGA;max_error_rate=0;min_overlap=20"..."TACCTTAGGGATAACAGCGTA\$;max_error_rate=0;min_overlap=21;" -o ${sample_id}.primers_trimmed.fastq $adapters_trimmed_2
	"""
}

// Step 3.1 - report quality control with fastqc
process fastqc_cutprimers {
        publishDir './output_16S_3.1_fastqc_cutprimers', mode: 'copy', overwrite: true
...
process multiqc_cutprimers {
        publishDir './output_16S_3.2_multiqc_cutprimers', mode: 'copy', overwrite: true
...
process filter {
	publishDir './output_16S_4.0_filtered', mode: 'copy', overwrite: true
...
process fastqc_filtered {
        publishDir './output_16S_4.1_fastqc_filtered', mode: 'copy', overwrite: true
...
process multiqc_filtered {
        publishDir './output_16S_4.2_multiqc_filtered', mode: 'copy', overwrite: true
...
process uniques {
	publishDir './output_16S_5.0_uniques', mode: 'copy', overwrite: true
...
process cluster_USEARCH {
	publishDir './output_16S_6.0_otus_USEARCH', mode: 'copy', overwrite: true
...
process otutab {
	publishDir './output_16S_7.0_otutab', mode: 'copy', overwrite: true
...
process matchlist {
	publishDir './output_16S_8.0_LULU', mode: 'copy', overwrite: true
...
process lulu {
	publishDir './output_16S_8.0_LULU', mode: 'copy'
	...
	"""
	Rscript --vanilla $PWD/lulu_16S.R otutab.txt matchlist.txt lulu_otus_id.txt lulu_result.csv
	"""
}

// Step 9.0 - BLAST against reference sequence database and output file for MEGAN
process blastn {
	publishDir './output_16S_9.0_MEGAN', mode: 'copy', overwrite: true
	...
	blastn -db $PWD/database_16S/Coleoptera_16S.fasta -query lulu_otus.fasta -evalue 0.001 -outfmt 5 -max_target_seqs 50 -out lulu_otus_MEGAN.txt -num_threads 12
	"""
}
